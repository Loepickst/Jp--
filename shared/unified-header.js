(function () {
    ...
    function ensureStylesheet() {
        if (document.querySelector("link[data-kiki-unified-header-style], link[href*='shared/unified-header.css']")) {
            return;
        }
        const link = document.createElement("link");
        link.rel = "stylesheet";
        link.href = new URL("unified-header.css?v=20260907-white-header", sharedBase).href;
    ...
    function returnTarget(config) {
        const direct = (value) => window.KikiRoutes
            ? window.KikiRoutes.canonicalize(value, window.location.href, new URL("../", sharedBase)).href
            : new URL(value, window.location.href).href;
        if (config.href) {
            return direct(config.href);
        }
        const params = new URL(window.location.href).searchParams;
        const returnValue = params.get("return");
        if (returnValue) {
            try {
                return direct(returnValue);
            } catch (error) {
    ...
    function shouldHideSource(source) {
        ...
        if (source.matches("body > header")) {
            if (source.hasAttribute("data-kiki-header-pending")) return true;
    ...
    function initUnifiedHeader() {
        let headerHost = document.querySelector("[data-kiki-unified-header-host]");
        if (headerHost?.dataset.kikiHeaderReady === "true"
            || document.querySelector(".kiki-unified-header:not([data-kiki-unified-header-host])")
        ...
        // Dynamic directories use the same node too; never insert a second topbar.
        if (!headerHost && source && shouldHideSource(source)) {
            headerHost = source;
            headerHost.dataset.kikiUnifiedHeaderHost = "page";
        }
        ...
        // Legacy height/position rules must not survive on the reused node.
        ...
        brand.innerHTML = `<img src="${logoUrl.href}" alt="kiji" width="96" height="44" decoding="async">`;
        ...
        header.removeAttribute("data-kiki-header-pending");
        header.dataset.kikiHeaderReady = "true";
    ...
    // Shells generated by scripts can finish their header in the same render task.
    window.KikiUnifiedHeader = { init: initUnifiedHeader };
